...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"frame"]) {
        ...
        CGFloat cgNum = self.slider.maximumValue - self.slider.minimumValue;
...
- (void)sliderAction:(UISlider *)slider
{
    ...
    if ((slider.maximumValue - slider.minimumValue)>0) {
        
        progress = slider.value / (slider.maximumValue - slider.minimumValue);
    }
...
- (void)setCache:(CGFloat)cache
{
    ...
    if (cgNum>0) {
        ...
        // 求出百分比
        progress = cache / (self.slider.maximumValue - self.slider.minimumValue);
    }
