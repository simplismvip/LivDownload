...
@property (nonatomic, weak) UIProgressView *volume;
...
/*
        ...
        share.hidden = YES;
        UIImage *imageShare = [UIImage imageNamed:@"navbar_share_icon_white"];
 ...
 */
        ...
        [play setImage:[UIImage imageNamed:@"navbar_pause_icon_white"] forState:UIControlStateNormal];
        [play setImage:[UIImage imageNamed:@"navbar_play_icon_white"] forState:UIControlStateSelected];
        play.hidden = YES;
        ...
        [next setImage:[UIImage imageNamed:@"navbar_nextvideo_icon_white"] forState:UIControlStateNormal];
        next.hidden = YES;
        ...
        */
        ...
        // 创建自己的音量条
        UIProgressView *volume = [[UIProgressView alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.height / 3.8, 30)]; // 先让slider横放
        ...
        for (UIView *view in volum.subviews) {
            if ([view isKindOfClass:[UISlider class]]) {
                ...
                // 把系统音量的值赋给自定义音量条
                self.volume.progress = self.volumeSlider.value;
            }
        }
        // 添加系统音量控件
        ...
        maxImageView.image = [UIImage imageNamed:@"navbar_sound_white_self"];
        ...
        minImageView.image = [UIImage imageNamed:@"navbar_mute_white_self"];
...
- (void)setupVideoByArray:(NSMutableArray *)array page:(NSInteger)page
{
    ...
    // 执行代理完成退出
    if ([self.delegate respondsToSelector:@selector(currentPLayPath:)]) {
        
        [self.delegate currentPLayPath:url.absoluteString];
    }
}
...
{
    // 给进度条赋值,当用户互动滑块的时候不去赋值, 这里不用 touchInside，因为touchInside有yes和no，松手后滑块有可能不走
    ...
    if ([self.begin.text isEqualToString:self.end.text]) {
...
{
    // 更改音量控件value
    self.volume.progress -= value / 10000; // 越小幅度越小
    
    // 更改系统的音量
    self.volumeSlider.value = self.volume.progress;
}
